...
namespace ego_planner
{
  PlanningVisualization::PlanningVisualization(ros::NodeHandle &nh)
  {
    ...
    optimal_list_path_pub = nh.advertise<nav_msgs::Path>("optimal_list_path",20);
  ...
  void PlanningVisualization::displayOptimalList(Eigen::MatrixXd optimal_pts, int id)
  {
    ...
    int pts_num = optimal_pts.cols();
    for (int i = 0; i < pts_num; i++)
    {
      geometry_msgs::PoseStamped pose_msg;
      pose_msg.pose.position.x = optimal_pts(0, i);
      pose_msg.pose.position.y = optimal_pts(1, i);
      pose_msg.pose.position.z = optimal_pts(2, i);
      path_msg.poses.emplace_back(pose_msg);
    }
    optimal_list_path_pub.publish(path_msg);
