...
namespace ego_planner
{
      ...
      const bool flag_randomPolyTraj, const bool touch_goal)
  {
    ...
    if (flag_success)
    {
      ...
      const auto &a_star_paths = poly_traj_opt_->getLastAStarPaths();
      if (!a_star_paths.empty()) {
        visualization_->displayAStarList(a_star_paths,
                                         9999); // 用一个固定的较大 id 避免冲突
      }

      poly_traj_opt_->visualizeDirections();

      Eigen::MatrixXd positions = best_MJO.getTraj().getPositions();
      std::vector<Eigen::Vector3d> waypoints;
      for (int i = 0; i < positions.cols(); ++i) {
        waypoints.push_back(positions.col(i));
      }
      // visualization_->displayInitPathList(waypoints, 0.15,
      //                                     1); // 用 ID=1 区分于初始轨迹
      ...
      poly_traj::MinJerkOpt &initMJO)
  {
    ...
    if (flag_first_call || flag_polyInit) /*** case 1: polynomial initialization ***/
    {
      ...
      else
      {
        ...
        innerPs.resize(3, 1);
        double time = traj_.global_traj.glb_t_of_lc_tgt - traj_.global_traj.global_start_time;
        double rand_time = time / 2 + (((double)rand()) / RAND_MAX - 0.5) * (time / 4);
        innerPs = traj_.global_traj.traj.getPos(rand_time) + (((double)rand()) / RAND_MAX - 0.5) * 1 * Eigen::Vector3d::Ones();
