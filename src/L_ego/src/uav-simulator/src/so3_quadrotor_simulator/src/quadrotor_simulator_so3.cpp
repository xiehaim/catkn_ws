...
static void
mavros_cmd_callback(const mavros_msgs::AttitudeTarget::ConstPtr& cmd)
{
  // convert thrust to force   todo
  command.force[0] = cmd->thrust;
  // command.force[1] =
  // command.force[2] =
  command.qx = cmd->orientation.x;
  command.qy = cmd->orientation.y;
  ...
  // config parameters from command  todo
  // command.kR[0]            = cmd->kR[0];
  // command.kR[1]            = cmd->kR[1];
  // command.kR[2]            = cmd->kR[2];
  // command.kOm[0]           = cmd->kOm[0];
...
main(int argc, char** argv)
{
  ...
  #ifdef USE_MAVROS
  ros::Subscriber mavros_cmd_sub =
    n.subscribe("mavros_cmd", 100, &mavros_cmd_callback, ros::TransportHints().tcpNoDelay());
  #endif
  ...
  #ifdef USE_MAVROS
  n.param("corrections/z", command.corrections[0], 0.0)
  n.param("corrections/r", command.corrections[1], 0.0);
  n.param("corrections/p", command.corrections[2], 0.0);

  n.param("gains/rot/x", command.kR[0], 1.5);
  n.param("gains/rot/y", command.kR[1], 1.5);
  n.param("gains/rot/z", command.kR[2], 1.0);
  n.param("gains/ang/x", command.kOm[0], 0.13);
  n.param("gains/ang/y", command.kOm[1], 0.13);
  n.param("gains/ang/z", command.kOm[2], 0.1);
  
  n.param("use_external_yaw", command.use_external_yaw, true);
  #endif
