...
#include "optimizer/opt.hpp" // 改为新的优化器头文件
...
public:
  ...
  inline double getSwarmClearance(void) {
    return poly_traj_opt_->get_swarm_clearance_();
  }
  inline int getCpsNumPrePiece(void) {
    return poly_traj_opt_->get_cps_num_prePiece_();
  }
  // inline PtsChk_t getPtsCheck(void) { return
  // poly_traj_opt_->get_points_check_(); }
...
private:
  ...
  PolyTrajOptimizerCeres::Ptr poly_traj_opt_; // 改为新的优化器类型
