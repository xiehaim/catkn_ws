...
void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh,
  ...
  nh.param("manager/use_multitopology_trajs", pp_.use_multitopology_trajs,
           false);
  ...
  poly_traj_opt_.reset(new PolyTrajOptimizerCeres);
...
bool EGOPlannerManager::reboundReplan(
  ...
  if (!computeInitState(start_pt, start_vel, start_acc, local_target_pt,
                        ...
                        ts, initMJO)) {
    ROS_ERROR("computeInitState failed");
    return false;
  }

  Eigen::MatrixXd cstr_pts =
      initMJO.getInitConstraintPoints(poly_traj_opt_->get_cps_num_prePiece_());
  vector<std::pair<int, int>> segments;
  if (poly_traj_opt_->finelyCheckAndSetConstraintPoints(
          segments, initMJO, true) == PolyTrajOptimizerCeres::CHK_RET::ERR) {
    ROS_ERROR("finelyCheckAndSetConstraintPoints failed");
    return false;
  }
  ...
  if (pp_.use_multitopology_trajs) {
    std::vector<ConstraintPoints> trajs =
        poly_traj_opt_->distinctiveTrajs(segments);
    ...
    for (int i = trajs.size() - 1; i >= 0; i--) {
      ...
      if (poly_traj_opt_->optimizeTrajectory(headState, tailState, innerPts,
        ...
        Eigen::MatrixXd ctrl_pts_temp =
            poly_traj_opt_->getMinJerkOpt().getInitConstraintPoints(
                poly_traj_opt_->get_cps_num_prePiece_());
        std::vector<Eigen::Vector3d> point_set;
    ...
    tailState << initTraj.getJuncPos(PN), initTraj.getJuncVel(PN),
        initTraj.getJuncAcc(PN);
    double final_cost;
    flag_success = poly_traj_opt_->optimizeTrajectory(
  ...
  if (flag_success) {
    ...
    setLocalTrajFromOpt(initMJO, touch_goal);
    ...
    cstr_pts = poly_traj_opt_->getMinJerkOpt().getInitConstraintPoints(
        poly_traj_opt_->get_cps_num_prePiece_());
...
bool EGOPlannerManager::computeInitState(
      ...
      flag_polyInit) /*** case 1: polynomial initialization ***/
  {
    ...
    if (!flag_randomPolyTraj) {
      ...
      innerPs.resize(3, 1);
      double time = traj_.global_traj.glb_t_of_lc_tgt -
                    traj_.global_traj.global_start_time;
      double rand_time =
          time / 2 + (((double)rand()) / RAND_MAX - 0.5) * (time / 4);
      innerPs =
          traj_.global_traj.traj.getPos(rand_time) +
          (((double)rand()) / RAND_MAX - 0.5) * 1 * Eigen::Vector3d::Ones();
      // innerPs = (start_pt + local_target_pt) / 2 +
      //           (((double)rand()) / RAND_MAX - 0.5) *
      //               (start_pt - local_target_pt).norm() *
      //               horizen_dir * 0.8 * (-0.978 / (continous_failures_count_
      //               + 0.989) + 0.989) +
      //           (((double)rand()) / RAND_MAX - 0.5) *
      //               (start_pt - local_target_pt).norm() *
      //               vertical_dir * 0.4 * (-0.978 / (continous_failures_count_
      //               + 0.989) + 0.989);
  ...
  } else /*** case 2: initialize from previous optimal trajectory ***/
  {
    if (traj_.global_traj.last_glb_t_of_lc_tgt < 0.0) {
      ROS_ERROR("You are initialzing a trajectory from a previous optimal "
                "trajectory, but no previous trajectories up to now.");
      return false;
    }

    /* the trajectory time system is a little bit complicated... */
    double passed_t_on_lctraj =
    ...
    for (int i = 0; i < piece_nums - 1; ++i) {
      if (t < t_to_lc_end) {
        ...
        ROS_ERROR("Should not happen! x_x 0x88 t=%.2f, t_to_lc_end=%.2f, "
                  "t_to_lc_tgt=%.2f",
...
bool EGOPlannerManager::setLocalTrajFromOpt(const poly_traj::MinJerkOpt &opt,
  ...
  bool ret = poly_traj_opt_->computePointsToCheck(
...
bool EGOPlannerManager::planGlobalTrajWaypoints(
  ...
  for (int j = 0; j < 2; ++j) {
    ...
    if (j == 2) {
      ROS_WARN("Global traj MaxVel = %f > set_max_vel",
