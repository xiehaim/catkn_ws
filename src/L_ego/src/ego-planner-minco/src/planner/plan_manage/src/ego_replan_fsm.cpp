...
namespace ego_planner
{
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    // try_sub_ = nh.createTimer(ros::Duration(0.05), &EGOReplanFSM::try2Callback, this);

    try_sub_ = nh.subscribe("/try", 1, &EGOReplanFSM::try2Callback, this);
    RM_sub_  = nh.subscribe("/mavros/rc/in", 1, &EGOReplanFSM::RM2Callback, this);
    ...
    broadcast_polytraj_pub_ = nh.advertise<traj_utils::MINCOTraj>("planning/broadcast_traj_send", 10);
    broadcast_polytraj_sub_ = nh.subscribe<traj_utils::MINCOTraj>("planning/broadcast_traj_recv", 100,
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case WAIT_TARGET:
    {
      ...
      {
        // ROS_WARN("!RM_msgs = %d",!RM_msgs);
        // ROS_WARN("have_save_goal = %d",have_save_goal);
        // if(!RM_msgs && have_save_goal)
        // {
        //     if (planNextWaypoint(save_goal_))
        //     {
        //       have_trigger_ = true;
        //       have_save_goal = false;
        //     }
      ...
      //if rm_msgs = 0 且 存在被中断的路径点位（截取为到达的目标点）
        // => 发布点位：：
                            // if (planNextWaypoint(点位))
                            // {  
                            //   清除点位
                            //   have_trigger_ = true;
                            // }
    ...
    {
      if(RM_msgs){
        changeFSMExecState(RM_CONTROL, "FSM");
      }else if (planFromLocalTraj(1))
      {
        changeFSMExecState(EXEC_TRAJ, "FSM");
      }else
        {
    ...
    case EXEC_TRAJ:
    {
      ...
      {
        std::cout << "touch_the_goal" << std::endl;
        have_target_ = false;
        have_trigger_ = false;
      ...
      {
        std::cout << "t_cur > replan_thresh_" << (t_cur > replan_thresh_ )<< std::endl;
        std::cout << "close_to_current_traj_end" << close_to_current_traj_end << std::endl;
        std::cout << "touch_the_goal" << touch_the_goal << std::endl;
        changeFSMExecState(REPLAN_TRAJ, "FSM");
        //刷新频率够快了 感觉不用多加一个这个
        // if(!RM_msgs){
        //   changeFSMExecState(REPLAN_TRAJ, "FSM");
        // }else changeFSMExecState(RM_CONTROL, "FSM");
      }
      break;
    }
    ...
    case RM_CONTROL:
    {
      ...
      else
      {
        if(!rm_flag)
        {
          ...
          have_target_ = false;
          have_trigger_ = false;
          std::cout << "RM_MSGS_GET" << std::endl;
          // //获取被阻断的点，或者获取下一个点
          // 需求不用保存上一个点位
          // save_goal_ = final_goal_;
          // have_save_goal = true;
          rm_flag = !rm_flag;
        }
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    static string state_str[8] = {"INIT", "WAIT_TARGET", "GEN_NEW_TRAJ", "REPLAN_TRAJ", "EXEC_TRAJ", "EMERGENCY_STOP", "SEQUENTIAL_START", "RM_CONTROL"};
  ...
  bool EGOReplanFSM::planNextWaypoint(const Eigen::Vector3d next_wp,
                                      const std::vector<Eigen::Vector3d>* internal_waypoints_ptr)
  {
    bool success = false;
    std::vector<Eigen::Vector3d> one_pt_wps;
    if(internal_waypoints_ptr!=nullptr)
    {
      one_pt_wps = *internal_waypoints_ptr;
    }
  ...
  void EGOReplanFSM::try2Callback(const std_msgs::BoolConstPtr &msg)
  // void EGOReplanFSM::try2Callback(const ros::TimerEvent &e)
  {
      RM_msgs = msg->data;
      // ROS_WARN("RM_C:%d",RM_msgs);
      // have_target_ = false;
      // have_trigger_ = false;
      // changeFSMExecState(WAIT_TARGET, "!!!!!!!"); //不行 仍然存在轨迹，状态机存在跳变

  }

  void EGOReplanFSM::RM2Callback(const mavros_msgs::RCInConstPtr &msg)
  { //down :: 1 AUTO
    RM_msgs = ((int)msg->channels[5] - 900) / 1000;
    //0.99 -0.01
    RM_msgs = !RM_msgs;
