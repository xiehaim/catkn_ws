...
namespace ego_planner
{
  ...
  bool PolyTrajOptimizer::computePointsToCheck(
      ...
      int id_cps_end, PtsChk_t &points_check)
  {
    points_check.clear();
    ...
    // points_check[0].push_back(pt_last);
    ...
    while (true)
    {
      if (t > DURATION)
      {
        if (touch_goal_ && points_check.size() > 0)
        {
          while (points_check.back().size() == 0)
          {
          ...
          {
            ROS_ERROR("Failed to get points list to check (0x02). points_check.size()=%d", (int)points_check.size());
            return false;
          }
          else
          {
        ...
        {
          ROS_ERROR("Failed to get points list to check (0x01). touch_goal_=%d, points_check.size()=%d", touch_goal_, (int)points_check.size());
          points_check.clear();
          return false;
        }
      }
      ...
      if (t < 1e-5 || points_check[id_cps_curr].size() == 0 || (pt - pt_last).cwiseAbs().maxCoeff() > RES_2)
      {
        points_check[id_cps_curr].emplace_back(std::pair<double, Eigen::Vector3d>(t, pt));
        pt_last = pt;
      }
